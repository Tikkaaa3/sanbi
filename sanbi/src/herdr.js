...
import { buildPiLaunchArgs } from "./skills.js";
import { runtimeTempRoot } from "./runtime-temp.js";
...
export function roleEnv(config, role) {
  ...
  return [
    ...
    `LAD_AGENT_PEER=${peer}`,
    `SANBI_RUNTIME_TEMP=${runtimeTempRoot(config.project.key)}`,
  ];
}
...
async function ensureAgent(session, adopted, role, paneId, agents) {
  ...
  const native = await buildPiLaunchArgs(adopted, role);
