...
import { claimNoGitWarning } from "../src/vcs.js";
...
try {
  ...
  if (options.command === "upgrade") {
    ...
    if (adopted.installedVersion === CURRENT_SCAFFOLD_VERSION || result.currentVersion === CURRENT_SCAFFOLD_VERSION) await migrateLegacySanbiSubagentToggle();
    if (result.changed) {
      ...
      if (result.createdDirectories.length) console.log(`Created ${result.createdDirectories.length} infrastructure director${result.createdDirectories.length === 1 ? "y" : "ies"}.`);
    ...
    if (adopted.vcs?.vcs === "none" && await claimNoGitWarning(adopted.root)) {
      console.warn("No Git repository detected.\n\nSanbi will work, but:\n- task delta attribution is degraded\n- review cannot use history/diffs reliably\n- rollback/auditability is reduced\n\nRecommended: initialize Git before serious development.");
    }
    ...
    await trustProject(adopted.root);
    if (adopted.installedVersion === CURRENT_SCAFFOLD_VERSION) await migrateLegacySanbiSubagentToggle();
